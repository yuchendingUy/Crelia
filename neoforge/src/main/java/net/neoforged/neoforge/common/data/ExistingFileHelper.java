...
public class ExistingFileHelper {
    ...
    public ExistingFileHelper(Collection<Path> existingPacks, final Set<String> existingMods, boolean enable, @Nullable final String assetIndex, @Nullable final File assetsDir) {
        ...
        for (Path existing : existingPacks) {
            ...
            if (!file.exists())
                continue;
            PackLocationInfo info = new PackLocationInfo(file.getName(), Component.empty(), PackSource.BUILT_IN, Optional.empty());
            PackResources pack = file.isDirectory() ? new PathPackResources(info, file.toPath()) : new FilePackResources.FileResourcesSupplier(file).openPrimary(info);
